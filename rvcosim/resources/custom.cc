...
static reg_t exit_insn(processor_t *p, insn_t insn, reg_t pc) {
  DUMP(INFO, spike) << fmt::format(
      "custom exit instruction at pc 0x{:08X} with code {}, switched to exiting mode.", pc,
      insn.u_imm());
...
struct : public arg_t {
  std::string to_string(insn_t insn) const { return std::to_string(insn.u_imm()); }
} exit_code;

std::vector<insn_desc_t> cosim_extension_t::get_instructions() {
  std::vector<insn_desc_t> insns;
  insns.push_back((insn_desc_t){0x7F, 0xfff, exit_insn, exit_insn, exit_insn, exit_insn, exit_insn,
                                exit_insn, exit_insn, exit_insn});
  return insns;
}
