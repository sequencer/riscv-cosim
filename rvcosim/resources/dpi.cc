...
DPI void instruction_fetch(IN svBitVecVal /* <32> */ *addr, OUT svBitVecVal /* <32> */ *data) {
  LOG(INFO) << fmt::format("[dpi]\t @{} rtl wants to fetch an instruction from address 0x{:08X}.",
...
DPI void load_store(IN svBitVecVal /* 32 */ *addr, IN svBitVecVal /* 32 */ *store_data,
  ...
  LOG(INFO) << fmt::format("[dpi]\t @{} rtl wants to {} memory at address 0x{:08X}", bridge.cycle(),
                           action, (uint32_t)*addr);
  if (write_enable)
    CHECK_S(false) << fmt::format("[dpi]\t @{} mem write is not yet implemented.", bridge.cycle());
  else
...
DPI void reg_file_write(IN svBit is_fp, IN svBit is_vector, IN svBitVecVal /* 5 */ *addr,
  ...
  LOG(INFO) << fmt::format("[dpi]\t @{} rtl wants to write {}#{} with data: 0x{:08X}",
...
DPI void init_cosim() {
  ...
  LOG(INFO) << fmt::format("[dpi]\t @{} dpi<->spike bridge initialized", bridge.cycle());
}
...
DPI void timeout_check() {
  ...
  TRY({
    if (bridge.cycle() > 50) {
      LOG(FATAL_S) << fmt::format("[dpi]\t @{} timeout, exiting...", bridge.cycle());
    }
  })
}
